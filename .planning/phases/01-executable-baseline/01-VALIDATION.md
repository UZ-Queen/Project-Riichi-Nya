...
| **Final project-test commands** | 같은 helper를 `-TestFilter MahjongRoundTraceTests`와 `-TestFilter SoloSessionLifecycleTests`로 각각 실행한다. 각 JSON `status`, `total`, `resultsPath`와 XML에서 trace 4개와 lifecycle 15개의 정확한 이름을 독립 검증한 뒤 결과를 `4 + 15 = 19`로 집계하고, `Phase1Build.BuildWindowsPlayer`를 별도 batch process로 실행한다. Live testable package의 자체 테스트가 포함되므로 unfiltered total은 Phase 1 project-test count로 사용하지 않는다. Test Runner와 build 모두 `-quit`를 전달하지 않는다. |
...
- **At plan 01-07:** `RecoverInterruptedSaveTest_RestoresDurableBackupBeforeNextMutation`가 fixture-owned 임시 경로에서 stale-backup과 originally-absent 복구를 같은 private helper로 검증한 뒤, 두 fixture를 분리 실행해 trace 4개와 lifecycle 15개의 고유 이름 및 zero non-passed를 확인하고 `4 + 15 = 19`로 집계한다. 이 자동 실행은 D-13 Player GUI 재관찰로 간주하지 않는다.
...
| V-07 | 01-07-T1/T2 | 7 | BASE-04/05 | T-01-38~41 | `RecoverInterruptedSaveTest_RestoresDurableBackupBeforeNextMutation` proves stale-backup restoration and originally-absent cleanup, while all three production-path persistence cases enter the same durable fixture guard | focused recovery + two filtered EditMode runs | focused helper GREEN + exact recovery case; trace helper GREEN + exact 4 unique passed XML cases; lifecycle helper GREEN + exact 15 unique passed XML cases; aggregate `4 + 15 = 19` | ✅ exists | ✅ green |
...
- [x] Plan 01-07 adds one deterministic two-branch save recovery regression, guards all three lifecycle persistence paths, and records the exact filtered `4 + 15 = 19` aggregate without changing production persistence or the D-13 human boundary.
