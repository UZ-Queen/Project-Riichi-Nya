...
**Gathered:** 2026-09-04
**Status:** Existing 8 plans revised 2026-09-06; P2-OPEN-01 resolved: initial 18, first draw 17; implementation not started
...
### Reusable Assets
- `Assets/Scripts/AL-1S/MahjongTileAndBlock.cs`: 문자열 패 fixture 생성과 기존 `MahjongTile` 값 타입을 재사용할 수 있다. 현재 `==`/`CompareTo`/hash는 `TileID` 기준이며 `Equals(object)`만 struct 전체 필드로 비교한다. 교정 대상은 API 의미 불일치이고 적5==일반5 자체나 서로 다른 값의 해시 충돌은 결함이 아니다.
- `Assets/Scripts/AL-1S/MahjongRound.cs`: `NewRound(seed, out player)`와 PRNG 주입 경계는 결정론적 패산 테스트에 재사용할 수 있다. `remainingTsumoCount`는 선언만 있고 실제 제한 흐름은 연결되지 않았다. `graveyard`는 현재 초기화되지만 solo의 상대 보관 책임은 제거 대상이다.
...
## 후속 정정과 미결 사항 — 2026-09-06

P2-CR-01~04는 `output/reviews/phase2-plan-change-requests.md`와 이번 사용자 적용 요청을 근거로 반영했다. 과거 선택 기록은 `02-DISCUSSION-LOG.md`에 보존하고 후속 정정을 추가한다. P2-CR-02의 계산 코어 검증은 02-04, 실제 솔로 지급 결함의 동일 테스트 RED/GREEN과 호출부 교정은 02-05가 소유한다.

**P2-OPEN-01 — RESOLVED (2026-09-06 후속 사용자 결정):** 기본값 18은 배패 후 첫 쯔모를 포함한 한 국의 허용 쯔모 총량이다. 배패 13장은 이 수량에서 차감하지 않고 첫 쯔모 직전 18로 초기화하며, 첫 패를 실제로 가져오면 18→17로 차감한 뒤 상태/표시에 전달한다. 조기 종료가 없으면 18번째 쯔모에서 1→0이 되고 같은 국의 19번째 쯔모는 없다. 새 국에서도 18로 초기화한 뒤 첫 쯔모 후 17이 된다. 마지막 패의 화료/최종 타패 분기는 그대로 유지한다. 결정 근거는 02-DISCUSSION-LOG의 후속 사용자 결정 기록이다.

멘젠쯔모 제외는 이번 수정 범위 밖이며 기존 `MenzenTsumo` catalog와 PLAYER-OBS-01의 6판 20부를 유지한다.
