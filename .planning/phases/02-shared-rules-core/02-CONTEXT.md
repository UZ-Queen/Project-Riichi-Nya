...
**Gathered:** 2026-09-04
**Status:** Existing 8 plans revised 2026-09-06; P2-OPEN-01 remains open before dependent implementation
...
**P2-OPEN-01:** 기본값 18이 배패 후 첫 쯔모를 포함한 총량인지, 첫 쯔모를 이미 가진 시점의 남은 패 수인지는 미결이다. 현 코드의 `remainingTsumoCount`는 선언만 있고 `GenerateYama`가 배패 뒤 `Tsumo`를 호출하므로 코드만으로 사용자 의도를 확정할 수 없다. 02-05 Task 2의 초기화·표시·총량 기대값을 구현하기 전에 사용자 확인과 결정 기록 갱신이 필요하다. 이 문서 수정은 끝부분의 1→0 계약만 확정한다.
