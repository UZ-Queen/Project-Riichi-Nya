...
- 결과: **네 요청의 문서 반영 완료. 8개 플랜 구조·정합성 검사 통과. P2-OPEN-01은 후속 사용자 결정으로 해소 (초기 18→첫 쯔모 후 17).**
...
| 02-05 | 02-04 | RULE-10. 실제 solo 연결, 지급 RED/GREEN, 남은 패·Haitei·국 진행, 개발 전용 관찰 setup. | SOL-PAY-01 실제 경로 금액/횟수, 1→0 마지막 패 분기·reset·4인 합성 caller. **초기 18→첫 쯔모 후 17/총 18회/같은 국 19번째 draw 없음/새 국 reset 검증을 추가.** |
...
| 02-08 | 02-07 | RULE-07/09/10. 실제 같은 PID 관찰과 증거 봉인. | 원래 01의 5항목 + 02~05 각 필수 결과와 직접 승인. 미관찰·실패 또는 확정된 초기화 계약의 검증 증거 누락은 Phase 완료/PASS로 봉인 불가. |
...
| 18 초기화 미결인데 자동 실행 가능으로 보일 위험 | 최초 검토에서 02-05 `autonomous: false`와 실행 전제를 명시했다. 후속 사용자 답으로 P2-OPEN-01을 해소하여 `autonomous: true`로 복원하고 초기 18→첫 쯔모 후 17 검증을 추가했다. 구현 미시작은 유지. |
...
검사 재실행: `node output/reviews/phase2-plan-revision-260906-mov/check-plans.cjs`. [최초 검사 결과](phase2-plan-revision-260906-mov/validation-results.json)는 당시 상태로 보존하고, [초기화 결정 반영 후 검사 결과](phase2-plan-revision-260906-mov/initialization-validation-results.json)에 이번 재검증을 기록했다. 수정 전 snapshot/편집 보조 스크립트는 프로젝트 로컬 검토 산출물이다. 제품 코드가 아니며 이 스크립트는 Unity를 실행하지 않는다.

## 남은 문제

**해소된 문제 — P2-OPEN-01:** 기본값 18은 배패 후 첫 쯔모를 포함한 한 국의 허용 쯔모 총량이다. 배패 13장은 이 수량에서 차감하지 않고 첫 쯔모 직전 18로 초기화하며, 첫 패를 실제로 가져오면 18→17로 차감한 뒤 상태/표시에 전달한다. 조기 종료가 없으면 18번째 쯔모에서 1→0이 되고 같은 국의 19번째 쯔모는 없다. 새 국에서도 18로 초기화한 뒤 첫 쯔모 후 17이 된다. CONTEXT/DISCUSSION-LOG/PLAN/RESEARCH/PATTERNS/VALIDATION 및 STATE를 갱신했다. 이전 미결 기록은 당시 이력으로 보존했다.

남아 있는 항목:
1. **기존 연구 추론과 후속 Phase 결정은 그대로 유지.** A1~A4의 작혼 세부 규칙은 기존 출처·dated inference 조건으로 남아 있다. Phase 3/4 요구사항의 과거 리치 경제·종료 규칙 문구 재조정은 이번 네 요청의 범위 밖이다. 새 규칙 결론을 만들지 않았다.
2. **실행 증거는 아직 없음.** 실제 RED/GREEN XML·정확한 Phase 2 case 수·빌드·GUI 관찰은 각 기존 플랜을 향후 승인된 범위에서 실행할 때 수집한다. 문서 검사 통과는 이 증거를 대신하지 않는다.

미반영한 승인 요청은 없다. P2-CR-03의 초기화 항목도 후속 사용자 결정으로 해소했다. 기존 계획 수정과 보고까지 완료하고 종료한다.
